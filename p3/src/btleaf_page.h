...
class BTLeafPage : public SortedPage {
	...
	public:
		...
		{
			KeyDataEntry addr;
			Datatype tmp; tmp.rid = dataRid;
			int length;

			make_entry(&addr, key_type, key, LEAF, tmp, &length);
		...
		{
			Status ret;
			if ((ret = firstRecord(rid)) != OK) return NOMORERECS;
			KeyDataEntry addr;
			int length;
			if ((ret = getRecord(rid, (char*)&addr, length)) != OK) return ret;
			Datatype tmp;
			get_key_data(key, &tmp, &addr, length, LEAF);
			dataRid = tmp.rid;
			return OK;
		}
